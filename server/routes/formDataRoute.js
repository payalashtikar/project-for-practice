...
router.post('/createuserformdata', async (req, res) => {
    try {
        const { firstName, lastName, address, gender, age, city, status, email, contact, profilePhoto, Date } = req.body;
        if (!firstName || !lastName || !address || !gender || !age || !city || !status || !email || !contact || !profilePhoto || !Date) {
